	...
	window.openThemeEditor = function ($mount, parentDialog, opts) {
      ...
      `);
        ...
        <div class="theme-editor-hint">
          ${
				tier === "basic"
					? __(
							"Showing the most-used controls. Switch to Advanced for the full palette."
					  )
      ...
      `);
		...
		const renderPalettes = ($pane) => {
			...
			const apply = (palettes) => {
				if (!palettes.length) {
					$pane.html(
						`<div class="theme-editor-hint">${escapeAttr(
							__("No palettes available.")
						)}</div>`
					);
					return;
				}
          ...
          <div class="theme-editor-hint">
            ${escapeAttr(
				__("Each palette is WCAG-AA verified. Click to fill all 11 colors at once.")
			)}
          </div>
        `);
        ...
        <div class="theme-gen">
          ...
          <div class="theme-editor-hint">
            ${escapeAttr(
				__(
					"Every color is derived from your brand color and checked against WCAG AA. Pick a variant to load it into the editor — you can still adjust anything by hand afterwards."
				)
			)}
          </div>
        </div>
      ...
      <button type="button" class="theme-gen-polarity-btn ${isOn ? "is-active" : ""}"
		...
		const paintGenResults = () => {
			...
			if (genBusy) {
				$r.html(
					`<div class="theme-palettes-loading">${escapeAttr(__("Generating…"))}</div>`
				);
				return;
			}
			if (genError) {
			...
			if (!genResults.length) {
				$r.html(
					`<div class="theme-editor-hint">${escapeAttr(
						__("No accessible palette could be derived from that color.")
					)}</div>`
				);
				return;
			}
      ...
      `);
        ...
        <button type="button" class="theme-palette-card ${selected ? "is-selected" : ""}"
          ...
          <div class="theme-palette-swatch"
            ...
            <div class="tps-row" style="background:${escapeAttr(c.bg_surface)} !important;">
              ...
              <span class="tps-text" style="color:${escapeAttr(
					c.text_primary
				)} !important;">Aa</span>
            </div>
            <div class="tps-btn"
          ...
          <div class="theme-palette-label">
            ...
            <span class="theme-palette-tag">${escapeAttr(
				p.category || (p.is_dark ? "Dark" : "Light")
			)}</span>
          </div>
        </button>`;
		};
		...
		const renderContrast = () => {
			...
			try {
				pairs = [
					{
						label: __("Text on background"),
						fg: v("text_primary"),
						bg: v("bg_primary"),
						target: 4.5,
					},
					{
						label: __("Text on surface"),
						fg: v("text_primary"),
						bg: v("bg_surface"),
						target: 4.5,
					},
					{
						label: __("Button text on button"),
						fg: v("button_text"),
						bg: v("button_bg"),
						target: 3.0,
					},
			...
			const rows = pairs.map((p) => {
				...
				try {
					...
					return { ...p, ratio: null, status: "unknown" };
				}
				const status =
        ...
        <div class="theme-contrast-badge is-${overall}">
          ...
          ${
				showFix
					? `<button type="button" class="tcb-fix btn btn-xs">${escapeAttr(
							__("Auto-fix")
					  )}</button>`
					: ""
			}
      ...
      `);
		...
		const autoFix = () => {
			...
			const ratioOnPrimary = (() => {
				try {
					return contrastRatio(tp, bp);
				} catch (_e) {
					return null;
				}
			})();
			const ratioOnSurface = (() => {
				try {
					return contrastRatio(tp, bs);
				} catch (_e) {
					return null;
				}
			})();
			...
			openSaveCustom: () => {
				...
				const d = new frappe.ui.Dialog({
					...
					primary_action: async (v) => {
						...
						if (!key) {
							frappe.show_alert({
								message: __("Name must contain letters or digits"),
								indicator: "orange",
							});
							return;
						}
						...
						try {
							const r = await frappe.call({
								...
								args: {
									...
									share_public: v.share_public ? 1 : 0,
								},
							});
							saved = r && r.message && r.message.name;
						} catch (err) {
							frappe.show_alert({
								message: __("Could not save theme: {0}", [
									(err && err.message) || "",
								]),
								indicator: "red",
							});
						...
						try {
							...
							frappe.show_alert({
								message: __("Saved, but it could not be applied: {0}", [
									(err && err.message) || "",
								]),
								indicator: "orange",
							});
