(function () {
			...
			const d = new frappe.ui.Dialog({
				...
				fields: [
					...
					},
					{
						...
						default:
							(pair.light && !pair.light.is_dark && pair.light.name) || undefined,
					},
					{
						fieldtype: "Select",
          ...
          <div class="tpm-body">
            ...
            <div class="tpm-content">
              ...
              <table class="tpm-table">
                ...
                <tbody>
                  <tr><td>Acme Corp</td><td><span class="tpm-pill">${escapeHtml(
						__("Paid")
					)}</span></td><td>$12,400</td></tr>
                  <tr><td>Globex</td><td><span class="tpm-pill tpm-pill-muted">${escapeHtml(
						__("Pending")
					)}</span></td><td>$3,200</td></tr>
                  <tr><td>Initech</td><td><span class="tpm-pill">${escapeHtml(
						__("Paid")
					)}</span></td><td>$7,890</td></tr>
                </tbody>
              </table>
	...
	function cardHTML(t, selectedName, kind) {
		...
		const deleteBtn =
			kind === "owned"
				? `<button type="button" data-action="delete" title="${__("Delete")}">${__(
						"Delete"
				  )}</button>`
				: "";
